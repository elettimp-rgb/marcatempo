...
function validaOre(ord, strF, strFe) {
  ...
  if (ord > 8) {
    return { ok: false, error: 'Le ore ordinarie non possono superare 8.' };
  }
  if (ord + strF + strFe > 14) {
    return { ok: false, error: 'Il totale (ordinario + straordinari) non puo superare 14 ore.' };
  }
  return { ok: true };
}
...
function apriModificaTimbratura(id, isAdmin) {
  ...
  $('btnSalvaModificaTim').onclick = function() {
    salvaModificaTimbratura(isAdmin);
  };
    ...
    function loadTimbratureAdmin() {
      ...
      callApi('/api/timbrature', { mese: mese }).then(function(r) {
        if (r && r.success) {
          ...
          tbody.innerHTML = '<tr><td colspan="15" class="text-center text-danger py-3">' + ((r && r.error) || 'Errore') + '</td></tr>';
        }
      });
    }
  ...
  (function() {
    ...
    function initCollab() {
      ...
      form.addEventListener('submit', function(e) {
        ...
        if (!check.ok) {
          showMessage('collabMessage', 'danger', check.error);
          return;
        }
